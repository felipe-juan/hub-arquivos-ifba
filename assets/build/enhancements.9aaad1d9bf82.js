(() => {
  ...
  let favoriteSyncFrame = 0;
  const queueFavoriteSync = () => {
    if (favoriteSyncFrame) return;
    favoriteSyncFrame = requestAnimationFrame(() => {
      favoriteSyncFrame = 0;
      syncFavoriteButtons();
    });
  };
  const observer = new MutationObserver(queueFavoriteSync);
