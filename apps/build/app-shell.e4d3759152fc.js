(() => {
  ...
  function inject() {
    ...
    const setMobileOpen = open => {
      document.body.classList.toggle("hub-shell-mobile-open", Boolean(open));
      mobileOpen?.setAttribute("aria-expanded", open ? "true" : "false");
    };
    mobileOpen?.addEventListener("click", () => setMobileOpen(!document.body.classList.contains("hub-shell-mobile-open")));
    ...
    window.addEventListener("resize", () => {
      if (!matchMedia("(max-width:920px)").matches) setMobileOpen(false);
    }, { passive: true });
